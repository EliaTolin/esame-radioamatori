...
def draw_simple_pll():
    ...
    d1 += elm.Line().down()
    d1 += elm.Ground()
    d1.save(OUTPUT_DIR / 'pll_base.svg')
    ...
    d2 += elm.Line().down()
    d2 += elm.Ground()
    d2.save(OUTPUT_DIR / 'pll_vco.svg')
...
def draw_pll_components():
    ...
    d1 += elm.Ground()
    d1.save(OUTPUT_DIR / 'phase_detector_pll.svg')
    ...
    d2 += elm.Ground()
    d2.save(OUTPUT_DIR / 'vco_pll.svg')
    ...
    d3 += elm.Ground()
    d3.save(OUTPUT_DIR / 'loop_filter_pll.svg')
...
def draw_pll_applications():
    ...
    d1.save(OUTPUT_DIR / 'sintetizzatore_pll.svg')
    ...
    d2.save(OUTPUT_DIR / 'clock_generator_pll.svg')
